...
class MoodleCourse():
    ...
    def from_course_id(cls, conn, course_id):
        ...
        params = {'field': 'id', 'value': str(course_id)}
        response = MoodleRequest(conn, 'core_course_get_courses_by_field').post(params).json()

        if response != []:
            assert(response['courses'][0]['id'] == course_id)
            fname = response['courses'][0]['fullname']
            sname = response['courses'][0]['shortname']
        else:
            fname = "<Unknown>"
            sname = "<Unknown>"
        return cls(conn=conn, course_id=course_id, fullname=fname, shortname=sname)
